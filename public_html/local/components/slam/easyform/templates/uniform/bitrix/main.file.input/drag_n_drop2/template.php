...
<div id="file-selectdialog-<?=$uid?>" class="file-selectdialog">
	...
	if($arParams['HIDE_FIELD_NAME']):
		?>
		<span class="file-label" style="display: none"><?=$arParams['INPUT_TITLE'] ? : GetMessage('BFDND_FILES')?></span>
	<?endif?>
		...
		<label class="">
			<?=GetMessage('BFDND_DROPHERE');?>
			<span class="file-uploader">
                <!--span class="file-but-text"><?=GetMessage('BFDND_SELECT_EXIST');?></span-->
		...
		<div class="file-selector">
            <span class="file-uploader">
                ...
                <span class="file-but-text">
                    <?=GetMessage('BFDND_SELECT_LOCAL');?>
                </span>
                <span class="file-uploader-right"></span>
                <input class="file-fileUploader <?=$addClass?>" id="file-fileUploader-<?=$uid?>" type="file" <?/*multiple='multiple'*/?> size='1' /></span>
        </div>
    </div>
	<script>
	BX.ready(function(){
