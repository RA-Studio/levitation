...
?>
	...
	<form class="email-subs email-subs_mobile" id="bx_subscribe_subform_<?=$buttonId?>" role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>">
		<?=bitrix_sessid_post()?>
        <div class="email-subs-wrap">
            ...
            <input pattern="([A-z0-9_.-]{1,})@([A-z0-9_.-]{1,}).([A-z]{2,8})" class="email-subs__email" type="email" required placeholder="Ваш E-Mail" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>">
            <button class="email-subs__btn" type="submit" id="bx_subscribe_btn_<?=$buttonId?>">Подписаться на рассылку</button>
    ...
    <div class="email-subs__success" id="subscribe_success_mobile" style="display: none">
        <p class="email-subs__text">Вы успешно <br>подписались на рассылку</p>
    </div>
    <script>
        BX.ready(function(){
            $('#subscribe_success_mobile').fadeIn().css('display', 'flex');
            setTimeout(function () {
                $('#subscribe_success_mobile').fadeOut();
            }, 1000);
        });
    </script>
    <?endif;?>
