...
<form class="lk-form subscribe" action="<?=$arResult["FORM_ACTION"]?>" method="get">
    ...
    <div class="lk-block__text">
        <?=GetMessage("subscr_act")?>
        <?=($arResult["SUBSCRIPTION"]["ACTIVE"] == "Y"? GetMessage("subscr_yes"):GetMessage("subscr_no"));?>
    </div>
    <?if(!empty($arResult["SUBSCRIPTION"]["DATE_UPDATE"])){?>
        <div class="lk-block__text">
            <?=GetMessage("subscr_date_upd")?>
            <?=$arResult["SUBSCRIPTION"]["DATE_UPDATE"];?>&nbsp;
        </div>
    <?}?>
        <?if($arResult["SUBSCRIPTION"]["CONFIRMED"] == "Y"):?>
            <?if($arResult["SUBSCRIPTION"]["ACTIVE"] == "Y"):?>
                <input class="lk-form__enter" type="submit" name="unsubscribe" value="<?echo GetMessage("subscr_unsubscr")?>" />
                <input type="hidden" name="action" value="unsubscribe" />
            <?else:?>
                <input class="lk-form__enter" type="submit" name="activate" value="<?echo GetMessage("subscr_activate")?>" />
