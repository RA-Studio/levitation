...
if(strlen($arResult["ID"])>0)
{
	...
	?>
	<form method="post"  class="lk-tab" id="lk-info3" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		<?=bitrix_sessid_post()?>
		<input type="hidden" name="ID" value="<?=$arResult["ID"]?>">
        <h4>
            <b><?= Loc::getMessage('SPPD_PROFILE_NO', array("#ID#" => $arResult["ID"]))?></b>
        </h4>
        <label class="lk-tab__label" for="sale-personal-profile-detail-name"><?=Loc::getMessage('SALE_PNAME')?>:&nbsp;<span class="req">*</span></label>
        <input class="lk-tab__input" type="text" name="NAME" maxlength="50" id="sale-personal-profile-detail-name" value="<?=$arResult["NAME"]?>" />
		<?
		foreach($arResult["ORDER_PROPS"] as $block)
		{
			if (!empty($block["PROPS"]))
			{
				...
				foreach($block["PROPS"] as $property)
				{
					...
					?>
						<label class="lk-tab__label" for="sppd-property-<?=$key?>">
							<?= $property["NAME"]?>:
							<?
							...
							elseif ($property["TYPE"] == "LOCATION")
							{
								...
								$locationClassName = 'lk-tab__input';
        ...
        <!--<button class="lk-tab__submit">Сохранить изменения</button>-->
			<input type="submit" class="lk-tab__submit" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
			<!--<input type="submit" class="lk-tab__submit"  name="apply" value="<?/*=GetMessage("SALE_APPLY")*/?>">-->
			<input type="submit" class="lk-tab__submit"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
